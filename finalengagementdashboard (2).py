...
DEFAULT_PATHS = [
    ...
    Path(r"C:\\Users\\akans\\Documents\\engagement_2021_2025_merged.csv"),  
]
...
def load_data(path: Optional[Path]) -> pd.DataFrame:
    ...
    df = df.rename(columns={
        ...
        "Time Engaged per Session": "time_engaged_per_session",
    })
    ...
    df["total_users"] = df["new_users"].fillna(0) + df["returning_users"].fillna(0)
    df["pct_returning"] = np.where(df["total_users"]>0, df["returning_users"]/df["total_users"]*100, np.nan)
    return df


uploaded = st.sidebar.file_uploader("Upload engagement_2021_2025_merged.csv", type=["csv"])  # for convenience
uploaded_path = None
if uploaded is not None:
    tmp = Path(st.experimental_get_query_params().get("tmpdir", ["/tmp"]) [0])
    tmp.mkdir(parents=True, exist_ok=True)
    uploaded_path = tmp / "_engagement_upload.csv"
    with open(uploaded_path, "wb") as f:
        f.write(uploaded.read())

with st.spinner("Loading data…"):
    df = load_data(uploaded_path)
...
with TAB1:
    if len(f):
        ...
        fig = go.Figure()
        fig.add_traces([
            go.Scatter(x=daily["date"], y=daily["total_users"], name="Total users",
                       mode="lines+markers" if show_points else "lines",
                       line=dict(width=3, color=PRIMARY), fill="tozeroy", fillcolor="rgba(37,99,235,.12)"),
            go.Scatter(x=daily["date"], y=daily["new_users"], name="New users",
                       mode="lines+markers" if show_points else "lines",
                       line=dict(width=2, color=GREEN)),
            go.Scatter(x=daily["date"], y=daily["returning_users"], name="Returning users",
                       mode="lines+markers" if show_points else "lines",
        ...
        fig2 = px.line(monthly, x="month_str", y="time_engaged_per_session", markers=show_points,
                       template=TEMPLATE, title="Monthly Avg Time Engaged per Session")
        ...
        fig3 = px.line(pr, x="month_str", y="pct_returning", markers=show_points,
                       template=TEMPLATE, title="% Returning Users Over Time")
...
with TAB2:
    ...
    if len(yearly):
        ...
        fig1 = px.bar(yearly, x="year", y=["new_users", "returning_users"], barmode="stack",
    ...
    if show_forecast and len(f):
        ...
        if PROPHET_AVAILABLE:
            ...
            try:
                ...
                fig_fc = px.line(forecast, x="ds", y="yhat", template=TEMPLATE,
        ...
        if not PROPHET_AVAILABLE:
            ...
            fig_fc2 = px.line(pd.concat([base, out]), x="date", y="yhat", template=TEMPLATE,
...
st.download_button(
    " Download filtered data (CSV)", f.to_csv(index=False).encode("utf-8"), "engagement_filtered.csv"
)

with st.expander("About this dashboard"):
    st.markdown(
        """
        **Engagement Trend Analysis (2021–2025)** — a refreshed, modern dashboard with:
